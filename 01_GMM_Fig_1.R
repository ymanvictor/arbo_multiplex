...
# --- Compile Stan model ------------------------------------------------------
mdl <- cmdstan_model(stan_file = "github_code/src/gmm_hybrid.stan")

# --- Data --------------------------------------------------------------------
data <- read_csv("github_code/raw_data/CHIKV_E2.csv") %>%
...
gmm_neg <- ggplot(df_neg, aes(x = x)) +
  ...
  xlab("log(MFI)") +
  scale_x_continuous(limits = c(MFI_min, MFI_max))
...
# --- Outputs -----------------------------------------------------------------
write_csv(params_k3_tbl, "output/gmm_posterior_stats_CHIKV_E2.csv")
write_csv(conv_diagn_k3, "output/gmm_ess_rhat_CHIKV_E2.csv")
