...
# --- Data --------------------------------------------------------------------
post_stats <- read_csv("output/gmm_posterior_stats_CHIKV_E2.csv") %>%
  rename(param = 1)

post_med <- setNames(post_stats$post_med, post_stats$param)

ctrls <- read_csv("data/CHIKV_E2.csv") %>% 
  filter(sampletype != "U")

aucs <- read_csv("data/empirical_vs_analytical_aucs.csv")
...
ggsave(fig_2, "figures/Fig_2.png", width = 15, height = 10, scale = 1.2, bg = "white")
