...
net_input <- readRDS("data/source_data_fig_5a.rds")
C_resid   <- net_input$C_resid
ant_cols  <- net_input$ant_cols
n_resid <- net_input$n_resid

# 2) PCA scores (PC1-3 + phenotype only)
pca_scores   <- readRDS("data/source_data_fig_5b.rds")

# 3) PCA loadings (long format: antigen, virus_group, PC, loading)
pca_loadings_long <- readRDS("data/source_data_fig_5c.rds")

# 4) Phenotype proportions by site × age-group (saved with base::save)
phenotype_props_age_site <- readRDS("data/source_data_fig_5d.rds")  # creates phenotype_props_age_site
