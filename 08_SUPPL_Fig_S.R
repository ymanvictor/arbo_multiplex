...
ab_df <- read_csv(file = "data/source_data_fig_s3.csv")

fig_s3 <- ab_df %>% 
  ggplot(aes(x = mfi, fill = site))+
  geom_histogram(alpha = 0.6)+
  geom_vline(aes(xintercept = seropos_threshold), linetype = "dashed")+
  scale_fill_viridis_d()+
  facet_wrap(~antigen, ncol = 7)+
  theme(legend.position = "bottom", legend.title = element_blank())

ggsave(plot_histogram_ab_site, file = "figures/Fig_S3.png", scale = 2, width = 6)
