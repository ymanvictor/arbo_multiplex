...
gmm_pars <- read_csv("data/Table_S1.csv")
...
aucs <- read_csv(file = "data/empirical_vs_analytical_aucs.csv")
...
ggsave(fig_s2, file = "figures/Fig_S2.pdf", scale = 1.2)

# ========================== Supplementary Fig S4B. =============================

# load FMM estimated thresholds

gauss_thresholds <- read_csv(file = "data/seropositivity_thresholds.csv") %>% 
  
  select(FMM_serothr_neg2sd, antigen) %>% 
  
  filter(!antigen %in% c("OROV_GC", "OROV_GN"))

wb_thresholds <- read_csv(file = "data/wb_seropositivity_thresholds.csv") %>% 
...
ggsave(plot_scatter_thr, file = "figures/Fig_S4B.pdf", scale = 1.2)
